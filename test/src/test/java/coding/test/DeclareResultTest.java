...
public class DeclareResultTest {
    ...
    public void testCase() {
        ...
        mySolution(id_list, report, k);
        System.out.println("------------------------");
        realSolution(id_list, report, k);
    }

    // 결과{muzi=2, neo=0, frodo=1, apeach=2}
    @Test
    public int[] mySolution(String[] id_list, String[] report, int k) {

        int[] answer = new int[id_list.length];
        Set<String> filter = new HashSet<String>(Arrays.asList(report));
        List<String> hh = new ArrayList<>();

        for (String a : id_list) {
            int count = 0;
            for (String s : filter) {
                if (a.equals(s.split(" ")[1])) {
                    count++;
                }
            }
            if (count >= k) {
        ...
        for (int i = 0; i < answer.length; i++) {
            for (String s : filter) {
                for (String h : hh) {
                    if (id_list[i].equals(s.split(" ")[0]) && s.split(" ")[1].equals(h)) {
                        answer[i] = answer[i] + 1;
                    }
                }
            }
